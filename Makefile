...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/bluechipd

build: go.sum clean
	go build -mod=mod $(BUILD_FLAGS) -o build/bluechip ./cmd/bluechipd
...
# Create log files
log-files:
	sudo mkdir -p /var/log/bluechipd && sudo touch /var/log/bluechipd/mund.log && sudo touch /var/log/bluechipd/bluechipd_error.log
